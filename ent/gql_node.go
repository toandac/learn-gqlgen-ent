...
func (a *Artcile) Node(ctx context.Context) (node *Node, err error) {
	node = &Node{
		ID:     a.ID,
		Type:   "Artcile",
		Fields: make([]*Field, 2),
...
func (c *Client) noder(ctx context.Context, table string, id int) (Noder, error) {
	switch table {
	case artcile.Table:
		n, err := c.Artcile.Query().
			Where(artcile.ID(id)).
			CollectFields(ctx, "Artcile").
			Only(ctx)
		if err != nil {
			return nil, err
		}
...
func (c *Client) noders(ctx context.Context, table string, ids []int) ([]Noder, error) {
	...
	switch table {
	case artcile.Table:
		nodes, err := c.Artcile.Query().
			Where(artcile.IDIn(ids...)).
			CollectFields(ctx, "Artcile").
			All(ctx)
		if err != nil {
			return nil, err
